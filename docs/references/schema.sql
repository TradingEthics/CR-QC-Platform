...
CREATE TABLE scoring_categories (
  id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  section         TEXT NOT NULL,              -- 'Critical Fail', 'Critical Error', etc.
  severity        error_severity NOT NULL,
  deduction       NUMERIC(5,2) NOT NULL,     -- Points to deduct (50, 15, 7.5)
...
SELECT
  ...
  COALESCE(err_stats.critical_count, 0) AS critical_count,
...
LEFT JOIN LATERAL (
  SELECT
    ...
    COUNT(CASE WHEN qe.severity = 'critical' THEN 1 END) AS critical_count,
...
GROUP BY a.id, a.name, i.name, err_stats.critical_fail_count, err_stats.critical_count, err_stats.significant_count, err_stats.major_count;
...
-- CRITICAL FAIL (−50)
('Critical Fail', 'critical_fail', 50, 'Error Leading to Loss of Business', NULL,
 'Agent provided incorrect information or behaved unprofessionally, which could result in a direct or potential loss of income. This also encompasses situations where agents provided inaccurate pricing information to clients or made payment-related mistakes impacting our company''s profitability.', 1),
...
('Critical Fail', 'critical_fail', 50, 'Zero Tolerance', 'Unauthorized Information Disclosure',
 'Agent disclosed sensitive information like tool specifics, personal contact details, or restricted data to clients. Such behaviors are strictly forbidden because they pose potential security and privacy hazards.', 3),

-- CRITICAL ERROR (−15)
('Critical Error', 'critical', 15, 'Incorrect Information', 'Incorrect Information',
...
('Critical Error', 'critical', 15, 'Incorrect Information', 'Incorrect FundedNext Branding',
...
('Critical Error', 'critical', 15, 'Incorrect Information', 'Context-Altering Spelling Errors',
 'If a spelling error substantially changes the meaning of the sentence, it will lead to a deduction. For instance, the difference between "HFT trading is not allowed" and "HFT trading is now allowed" can result in such deductions.', 12),

('Critical Error', 'critical', 15, 'Lack of Professionalism', 'Inappropriate/Unprofessional Interaction',
 'Agent exhibited unprofessional conduct when dealing with clients, which encompassed behaviors like being impolite, using sarcasm, using abusive language, or adopting a condescending tone.', 13),

('Critical Error', 'critical', 15, 'Lack of Escalation/Unnecessary Escalation', 'Lack of Escalation',
 'Agent failed to escalate an issue that clearly requires attention or intervention from other stakeholders. This can result in prolonged resolution times and increased customer frustration.', 14),

('Critical Error', 'critical', 15, 'Lack of Escalation/Unnecessary Escalation', 'Unnecessary Escalation',
 'Agent chose to escalate a case to other stakeholders even when they possessed the capability to resolve the issue independently. Unwarranted escalations can result in inefficiencies and increased workload.', 15),

('Critical Error', 'critical', 15, 'Lack of Escalation/Unnecessary Escalation', 'Incorrect Escalation Channel/Team Assignment',
...
('Critical Error', 'critical', 15, 'Lack of Escalation/Unnecessary Escalation', 'Neglected Escalation History Verification',
 'Agent failed to review the prior escalation records, which resulted in the submission of a duplicate case, leading to delays in resolving the matter.', 17),

('Critical Error', 'critical', 15, 'Lack of Escalation/Unnecessary Escalation', 'Insufficient Information During Escalation',
 'Agent concluded an interaction and initiated a case escalation without providing the essential information needed to address the problem.', 18),

('Critical Error', 'critical', 15, 'Lack of Escalation/Unnecessary Escalation', 'Exaggerated Engagement',
 'If there is a situation where a double payment occurs, the agent proactively offered a refund without waiting for the client to raise the issue.', 19),

('Critical Error', 'critical', 15, 'Lack of Investigation', 'Lack of Investigation',
 'The agent closed the ticket or email without thoroughly examining the client''s concern while overlooking some important details. Verify that all client issues are thoroughly investigated before responding or escalating.', 20),

('Critical Error', 'critical', 15, 'Lack of Empathy', 'Apology Spiel Missing',
 'Agent didn''t offer an apology tailored to the client''s issue, particularly when the problem was caused by FundedNext. This includes situations like server errors, dashboard glitches, etc.', 21),

('Critical Error', 'critical', 15, 'Lack of Empathy', 'Empathy Spiel Missing',
 'Agent failed to express any kind of empathy for any inconvenience the client is encountering, regardless of whether the issue originated from our end or not.', 22),

('Critical Error', 'critical', 15, 'Lack of Empathy', 'Mismatched Emotional Tone',
 'Instead of displaying an appropriate emotional tone that acknowledges the customer''s feelings, agent''s response might come across as indifferent, robotic, or even dismissive.', 23),

-- SIGNIFICANT ERROR (−15)
('Significant Error', 'significant', 15, 'Grammatical Errors', 'Grammatical Mistakes',
 'Instances where the agent commits grammatical errors in their responses.', 30),

('Significant Error', 'significant', 15, 'Grammatical Errors', 'Spelling Mistakes',
 'Instances where the agent commits spelling errors in their responses.', 31),

('Significant Error', 'significant', 15, 'Grammatical Errors', 'Typos',
 'Instances where the agent commits typographical errors in their responses.', 32),

-- MAJOR ERROR (−7.5)
('Major Error', 'major', 7.5, 'Missing Information', 'Missing Information',
 'Agent provided a response that did not fully address all aspects of the client''s query. When a client has multiple questions and the agent addresses most of them but misses one query.', 40),

('Major Error', 'major', 7.5, 'Outdated Macro Usage', 'Outdated or Inadequate Macro Usage',
 'Agent used a macro that needed editing, paraphrasing, or utilized an outdated macro that failed to adequately acknowledge or resolve the client''s problem.', 41),

('Major Error', 'major', 7.5, 'Information Overload', 'Information Overload',
 'Agent did not tailor their responses to match the specific query and included irrelevant details.', 42),

('Major Error', 'major', 7.5, 'Inadequate/Excessive Engagement', 'Overcomplication of Responses',
 'Agent offered accurate yet unclear and complicated explanations that clients might struggle to comprehend.', 43),

('Major Error', 'major', 7.5, 'Inadequate/Excessive Engagement', 'Inadequate/Excessive Engagement',
 'Balancing customer interaction is crucial, as agents should provide an appropriate level of engagement, neither giving too little assistance nor overwhelming them.', 44),

('Major Error', 'major', 7.5, 'Inadequate/Excessive Engagement', 'Overutilization of ChatGPT/Automation Tools',
 'Agent did not maintain a personalized tone in responses to avoid coming across as overly robotic or driven by AI.', 45),

('Major Error', 'major', 7.5, 'Missing Minimal Information', 'Missing Minimal Information',
 'Agent offered accurate information but failed to provide sufficient context and a detailed explanation, which could result in the client not being fully informed.', 46);
